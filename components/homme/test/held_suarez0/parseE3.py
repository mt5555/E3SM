...
try:
    ...
    plt.axis([min(time), max(time)+5, 0, 2.5e9])
    plt.grid(True)
    plt.legend()
    ...
    #plt.axis([0, 700, -.5, .5])
    #plt.axis([min(time), max(time)+5, -.6,.2])
    #plt.axis([min(time), max(time)+.1, -1.,1.])
    plt.grid(True)
    plt.legend()
    ...
    legend1=("TBOT min: %.3f std: %.4f" % (min(Tmin),np.std(Tmin)) )
    plt.plot(time,Tmin,label=legend1)
    legend1=("TBOT max: %.3f std: %.4f" % (max(Tmax),np.std(Tmax)) )
    plt.plot(time,Tmax,label=legend1)
    #plt.axis([min(time), max(min(time)+200,max(time)+10), 0,400])
    plt.axis([min(time), max(time)+5, 0,400])
    plt.grid(True)
    plt.legend()
